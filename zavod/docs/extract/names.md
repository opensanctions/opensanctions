...
We use [DSPy](https://dspy.ai/) to write, optimise, and evaluate the prompt, but we use the resulting prompt directly (not via DSPy) so that DSPy doesn't become a production ETL dependency. The optimised prompt is loaded at runtime from `zavod/zavod/extract/names/single_entity_program.json`.

The tooling for optimising and evaluating the prompt lives outside the `zavod` package, in `contrib/prompt_tuning/`. See its [README](https://github.com/opensanctions/opensanctions/blob/main/contrib/prompt_tuning/README.md) for how to run it.
