...
# Baseline assertions applied to every non-collection dataset. They can be
# overridden per-dataset in the `assertions:` YAML block: because the merge
# happens at the config-dict level (see `merge_assertions_config`), a dataset
# that sets e.g. `min.property_fill_rate.Person.name` replaces the default
# threshold for that exact schema/property while keeping the rest. Lower a
# threshold to 0 to effectively disable a `min` default.
#
...
class Dataset(FollowTheMoneyDataset):
    def __init__(self, data: Dict[str, Any]):
        ...
        # Baseline default assertions apply to source datasets only; collections
        # aggregate other datasets and shouldn't carry per-schema fill-rate checks.
        ...
        # The "no assertions" nudge is about what the maintainer configured, not
        # the defaults we inject, so track user-provided assertions separately.
        self.has_user_assertions: bool = (
            len(list(parse_assertions(user_assertions_config))) > 0
        )
        assertions_config = (
            merge_assertions_config(DEFAULT_ASSERTIONS, user_assertions_config)
            if not self.is_collection
            else user_assertions_config
        )
