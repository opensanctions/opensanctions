...
def test_run_dataset():
    ...
    result = runner.invoke(run, [DATASET_YML.as_posix()])
    assert result.exit_code == 0, result.output
...
def test_load_db():
    ...
    assert result.exit_code != 0, result.output
    result = runner.invoke(load_db, [DATASET_YML.as_posix(), db_uri])
    assert result.exit_code == 0, result.output
...
def test_dump_file():
    ...
    assert result.exit_code != 0, result.output
    result = runner.invoke(dump_file, [DATASET_YML.as_posix(), out_path.as_posix()])
    assert result.exit_code == 0, result.output
