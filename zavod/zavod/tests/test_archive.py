...
def test_archive_publish(testdataset1: Dataset):
    ...
    # filesystem backend absoute path
    fs_archive_path = settings.DATA_PATH / settings.ARCHIVE_BUCKET / DATASETS
    assert not local_path.exists()
    with open(local_path, "w") as fh:
        fh.write("hello, world!\n")
    archive_path = fs_archive_path / settings.RELEASE / testdataset1.name / name
    assert not archive_path.exists()
    publish_resource(local_path, testdataset1.name, name, latest=False)
    assert archive_path.exists()

    latest_path = fs_archive_path / "latest" / testdataset1.name / name
    ...
    shutil.rmtree(fs_archive_path / "latest")
