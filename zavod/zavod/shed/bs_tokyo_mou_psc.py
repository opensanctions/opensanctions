...
def crawl_vessel_page(
    ...
    # Sometimes the source serves a body that's missing the expected tables. When
    # that happens the parse below raises ValueError, so we evict the cache entry
    # and retry, forcing the next attempt to re-fetch a fresh page from source.
