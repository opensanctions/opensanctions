...
def crawl_row(context: Context, str_row, id: str, name, url):
    ...
    date = str_row.pop("Date")
    party = str_row.pop("Party")
    description = str_row.pop("Description")

    person = context.make("Person")
    person.id = context.make_slug(id)
    person.add("name", name)
    person.add("description", description)
    if "election" in person.get("description")[0].lower():
        start_date = date
    ...
    context.audit_data(str_row, ["Costituency"])


def crawl(context: Context):
    doc = context.fetch_html(context.data_url, cache_days=3)
    ids = doc.xpath(
        ".//select[@id='ctl00_ContentPlaceHolder1_dmps_mpsListId']/option/@value"
    )
    names = doc.xpath(
        ".//select[@id='ctl00_ContentPlaceHolder1_dmps_mpsListId']/option/text()"
    )
