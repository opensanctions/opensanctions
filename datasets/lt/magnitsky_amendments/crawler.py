...
def check_data_limits(data):
    ...
    if page_size > 400:
        raise ValueError(
            f"Error: pageSize ({page_size}) is greater than the pre-set limit of 400."
        )
...
def crawl(context: Context):
    ...
    for entry in data.get("list", []):
        ...
        person.id = person_id
        person.add("name", f"{entry.get('vardas')} {entry.get('pavarde')}")
        ...
        for nationality in nationalities:
            ...
            if code:
                person.add("nationality", code)

        # Create a Sanction entity
        sanction = h.make_sanction(context, person)
        sanction.add("reason", entry.get("priezastis"))
