...
def extract_data(ele):
    ...
    for p in ele:
        ...
        if strong_tag is not None:
            ...
            data_dict[current_key] = p.xpath('string()').replace(strong_tag.text_content(), '').strip()
...
def parse_period(period: str) -> Tuple[str, str]:
    ...
    period = period.replace("\xa0", " ").replace("From ", "").replace(" to ", " ").replace(" To ", " ")
...
def crawl_item(item: Dict[str, str], context: Context):
    ...
    if item["Date of Birth"] != "Not known":
        person.add("birthDate", h.parse_date(item.pop("Date of Birth"), formats=["%d %B %Y"]))
    else:
    ...
    if "Dates of Disqualification" in item:
        ...
        sanction.add("startDate", h.parse_date(start_date, formats=["%d %B %Y", "%d %b %Y"]))
        sanction.add("endDate", h.parse_date(end_date, formats=["%d %B %Y", "%d %b %Y"]))
    ...
    if "Particulars of Disqualification Order or Undertaking" in item:
        sanction.add("reason", item.pop("Particulars of Disqualification Order or Undertaking"))

    context.emit(person, target=True)
    context.emit(sanction)

    context.audit_data(item, ignore=["Roles in respect of the Disqualification Order or Undertaking", "Means of Disqualification"])
