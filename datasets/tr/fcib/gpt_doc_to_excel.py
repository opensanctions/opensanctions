...
{", ".join(COLUMNS)}. When adding a name, omit structures like this :"•	Adı  						:" or "136-Adı Soyadı". 
Store the original name into the "original_name", and trimmed name into the "name". 
Multiple values should be separated by newlines without numbering
or bullet points. If a field is blank, the value should be null. Each distinct entry for 
each column should be on a single line. If you think there's a problem with the data, 
e.g. split in half or two persons combined, add a short note about that in the "issues" 
field, otherwise leave it blank. If more than one alias is given,  place one on each line. 
If you encounter "ValueError:", add a note in the "issues" field, and leave the field blank. 
"""

infile = sys.argv[1]
persons = open(infile).read().split("\n\n")
