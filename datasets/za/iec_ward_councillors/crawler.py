...
def crawl_muni(context: Context, form_data: Dict[str, str], muni_btn: str) -> None:
    ...
    form_data["ctl00$ctl13"] = (
        "ctl00$MainContent$MainUpdatePanel|ctl00$MainContent$uxWardCountMunicipalListView$ctrl0$ctl00$uxMunicLinkButton"
    )
    ...
    for rownum, row in enumerate(table.findall(".//tr")):
        if headers is None:
            headers = [
                slugify(el.text_content(), sep="_") for el in row.findall("./th")
            ]
            continue
...
def crawl_province(
    context: Context, form_data: Dict[str, str], province_input: str
) -> None:
    ...
    muni_btn_xpath = (
        './/div[@id="MainContent_uxWardCountMunicipalView"]//input[@type="submit"]'
    )
