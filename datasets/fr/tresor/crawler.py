...
def parse_split(context: Context, entity: Entity, full: str):
    ...
    indices = sorted([full.find(key) for key in TEXT_KEYS.keys()])
    indices = [i for i in indices if i != -1]
    segments = [full[i:j] for i, j in zip(indices, indices[1:] + [None])]

    cleaned_segments = []
    for segment in segments:
        segment = segment.rstrip(SEPARATORS)
        if segment != "":
...
def parse_identification(
    ...
    for segment in segments:
        ...
        for key, propname in TEXT_KEYS.items():
            if segment.startswith(key) and propname is not None:
                value = segment.removeprefix(key).strip(SEPARATORS)
