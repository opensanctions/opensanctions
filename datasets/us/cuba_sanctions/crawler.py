...
PROGRAM_KEY_REA = "US-DOS-CU-REA"
PROGRAM_KEY_PAL = "US-DOS-CU-PAL"
...
def crawl_accommodations(context: Context):
    ...
    with open(path, "r") as fh:
        for row in csv.DictReader(fh):
            ...
            context.emit(proxy)
            sanction = h.make_sanction(context, proxy, program_key=PROGRAM_KEY_PAL)
...
def crawl_restricted_entities(context: Context):
    ...
    with open(path, "r") as fh:
        for row in csv.DictReader(fh):
            ...
            sanction = h.make_sanction(context, proxy, program_key=PROGRAM_KEY_REA)
