...
def parse_table(
    ...
    return {row.findtext(".//th"): row.findtext(".//td") for row in table.findall(".//tr")}
...
def crawl_item(url: str, context: Context):
    ...
    if "Date of Birth" in info_dict:
        entity.add("birthDate", h.parse_date(info_dict.pop("Date of Birth"), formats=["%d-%m-%Y"]))
    ...
    sanction = h.make_sanction(context, entity, key=info_dict.pop("Notification Number"))
