...
COUNTRIES = {
    ...
    "სირიისარაბთა რესპუბლიკა",  # Syrian Arab Republic
    "ბადრიიეჰ იუნესი",  # Badriyeunesi
    "სამხრეთაფრიკა",  # South Africa
    "შპს აიდიესბორჯომი ბევერიჯიზ კომპანის საქართველოს",  # LLC Aidesborjom Beveridge Company Georgia
    "სინგაპური",  # Singapore
    "რიადჰ აბდულაჰ მ",  # Riyadh Abdulah M
    "გაერთიანებული სამეფო",  # United Kingdom
    "არაბთაგაერთიანებული საემიროები",  # United Arab Emirates
    "ლუქსემბურგის დიდი საჰერცოგო",  # Grand Duchy of Luxembourg
    "კორეის სახალხო დემოკრატიული რესპუბლიკა",  # Democratic People's Republic of Korea
    "დიდიბრიტანეთისა და ჩრდილოეთ ირლანდიისგაერთიანებული სამეფო",  # United Kingdom of Great Britain and Northern Ireland
    "კამერუნი",  # Cameroon
    "ზამბია",  # Zambia
    "ჰანაა მეგლაა ბოტროსაბდელმესსიჰ",  # Hanna Meglaa Botrosabdelmessih
    "ალბერტ ვაჰბა მაკარ მათთა",  # Albert Vahba Makar Matta
    "გიბრალტარი",  # Gibraltar
    'ნოვა ბილდინგ"',  # Nova Building
    "ჩეხეთისრესპუბლიკა",  # Czech Republic
    "ნასერ ესკანდარი",  # Naser Eskandari
    "აჰმად აბდულრაჰმან ჰ",  # Ahmad Abdulrahman H
    "თურქეთი ()",  # Turkey
    "საორგანიზაციო მდივანი",  # Organizational Development
    "ირანი (ისლამურირესპუბლიკა)",  # Iran (Islamic Republic)
    "ესტონეთი",  # Estonia
    "ისრაელისაქართველო",  # Israel, Georgia
    "კუნძული მენი",  # Kunzuli Meni
    "სართული",  # Stairs
    "სირიის არაბთა",  # Syrian Arab
    "ХТИ №",  # HTI #
    "როუდ თაუნი",  # Roud Tauni
    "ნარგეს მაჰერ ისააქ კეჰერ",  # Narges Maher Isaak Kehrer
    'სუხიშვილის უნივერსიტეტი" (ყოფილიშ პ ს',  # Sukhishvili University (Kopilish P S
    "აშშ",  # USA
    "ბელიზი",  # Belize
    "კურასაო",  # Curacao
    "ბაჰრეინი მანამა",  # Bahrain Manama
    "ბელგია",  # Belgium
    "აიტეკე ბი ქ # VIსარ",  # Aiteke B Street # VI
    "აფხაზეთი",  # Abkhazia
    "თავმჯდომარის მოადგილე",  # Tavmjdomari's Moaadgile
    "ფერსტ აილენდ",  # First Island
    "გამზ",  # Gamz
    "ისრაელი ()",  # Israel
    "სალე ბეირამი",  # Sale Beiram
    "ეთიოპია",  # Ethiopia
    "მარიამ ბებურიშვილი",  # Mariam Beburishvili
    "მალტა",  # Malta
    "ლუდოვიკ ბელ",  # Ludovic Bel
    "კორეის სახალხო დემოკრატიული რე",  # Democratic People's Republic
    "ბურკინა ფასო",  # Burkina Faso
    "ლიეტუვა",  # Liechtenstein
    "ჰანი გჰავიდელ",  # Hani Ghavidel
    'დიდგორი"',  # Didgori
    "ესპანეთი",  # Spain
    "ფრანსის დრეიკის გზატ",  # France's Dreiki Road
    "ჰოლანდია",  # Holland
    'ვიქტორია"',  # Victoria
    "ალირეზა აბჯადპურ",  # Alireza Abjadpur
    "სენტ-ვინსენტი და გრენადინები",  # Saint Vincent and the Grenadines
    "ჯოელი ტროშანი",  # Joel Troshani
    "სენტკიტსი და ნევისი",  # Saint Kitts and Nevis
    "გერმანია",  # Germany
    "ლიბიის არაბთა ჯამაჰირია",  # Libya's Arab Jamahiriya
    "დე კასტროს ქ",  # De Castro's Street
    "s r o",  # s r o
    "დიდიბრიტანეთისა და ჩრდილოეთირლანდიის გაერთიანებულისამეფო",  # United Kingdom of Great Britain and Northern Ireland
    "ეგვიპტე",  # Egypt
    'ალაფიაგრო"',  # Alaphagro
    "აბდულჰადი აბდულრაბალრასოლ მ",  # Abdulhadi Abdulrabalrasol M
    "სიამაკ პოურია",  # Siamak Pouria
    "მე სართ  #",  # Me Sart #
    "საბერძნეთი",  # Greece
    "საფრანგეთი",  # France
    "ბენქინგ დისტრიქტ",  # Banking District
    "მარი ეზზატ ფაიაზ მატიას",  # Mari Ezzat Fayaz Matias
    "სუბტროპიკული კულტურებისა და ჩაისმრეწველობის ინსტიტუტი",  # Subtropical Cultures and Conservation Institute
    "ვიეტნამი",  # Vietnam
    #############################################
    "თურქეთი",  # Turkey
    "მარკ  სქვეა",  # Mark Skwea
    "გოლამრეზა დამერჯი მაჰმუდი",  # Golamrezadamergi Mahmodi
    "მდივანი",  # Development
    "ჩეხეთი",  # Czech
    "ჩადზიპავლოუ",  # Chadzipavlo
    "დიდი ბრიტანეთისა და ჩრდილოეთ ირლანდიის",  # United Kingdom of Great Britain and Northern Ireland
    "მილანი",  # Milan
    "ჰამიდ ნაფარ",  # Hamid Napar
    "ბენინი",  # Benin
    "მოჰამმად რეზა შირაზი",  # Mohamed Reza Shirazi
    "აზარბადეგან",  # Azerbaijan
    "უგანდა",  # Uganda
    "ვირჯინიის კუნძულები(ბრიტ )",  # Virgin Islands (Brit)
    "თავმჯდომარის პირველი მოადგილე",  # Tavmjdomari's First Moaadgile
    "ელიზ კოლცზ",  # Eliz Kolz
    "არაბთა",  # Arab
    "კანადა",  # Canada
    "ვიხამს ქეი",  # Vixams Kay
    "სალეჰ ჰამად ეი",  # Saleh Hamad A
    "გვინეა-ბისაუ",  # Gvineau-Bisau
    "გარემოს დამცველი",  # Garemo's Defender
    "ზიმბაბვე",  # Zimbabwe
    "სირიის არაბთა რესპუბლიკა",  # Syrian Arab Republic
    "ვირჯინიის კუნძულები (ბრიტ.)",  # Virgin Islands (Brit.)
    'პოიკერ და ნებელ"',  # Poker and Nebel
    "სომალი",  # Somalia
    "(საქართველო)",  # (Georgia)
    "მოჰამმედ აჰმედ ჯ",  # Mohamed Ahmed J
    "რუსეთის",  # Russia's
    "რობერტ",  # Robert
    "დანია",  # Denmark
    "შრილანკა",  # Sri Lanka
    'ინაკო"',  # Inaco
    "პროკურისტი",  # Procurist
    "სულემან საუდ ს",  # Suleiman Saud S
    "ჰოლსველ როუდ ლონდონი",  # Holsvel Road London
    "ლიბია",  # Libya
    "ამერიკის სამოა",  # America's Samoa
    "ეი ფი",  # A F
    "ვანუატუ",  # Vanuatu
    "უკრაინა ()",  # Ukraine
    "პალაუ",  # Palau
    "აბდულრაჰმან მაჰმუდ ჰ",  # Abdulrahman Mahmoud H
    "სენტ-კიტსი და ნევისი",  # Saint Kitts and Nevis
    "ნიგერი",  # Niger
    "დიდი ბრიტანეთისა და ჩრდილოეთ",  # United Kingdom of Great Britain and Northern
    "დომინიკელთა რესპუბლიკა",  # Dominican Republic
    "აიტეკები VI სართ",  # Aitekebi VI Street
    "ვირჯინიის კუნძულები (ბრიტ )",  # Virgin Islands (Brit)
    "მოჰამმად ხოდაიარი",  # Mohamed Khodaiari
    "მევქიის რი თერმე",  # Mevki's R Term
    "მაჰერ ლაბიბ საად მოავად",  # Maher Labib Saad Movaad
    "რუმინეთი",  # Romania
    "ხორვატია",  # Croatia
    "დ უმირზაკრგპ ტერიტორია",  # D Umirzakrgp Territory
    "კოსოვო",  # Kosovo
    "არაშ შაჰფარი",  # Arash Shahpari
    "ჩინეთი",  # China
    "დან",  # Dan
    "ნაუოზის",  # Nauozis
    "მურათ იღით ჩალიქ",  # Murat Igit Chalik
    "კორეის რესპუბლიკა",  # Republic of Korea
}
