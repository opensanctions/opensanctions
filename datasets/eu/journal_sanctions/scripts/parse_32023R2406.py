"""Parse consolidated Regulation (EU) 2023/2406 (Niger) into a CSV.
...
- Annex I — the Article 2 fund-freeze list, which has never held a
  designation: it prints its subtitle and one "[…]" placeholder paragraph.
  The parser accepts exactly that placeholder and breaks the day the
  Council designates anyone, so the entry structure gets taught under
  review. Travel bans live in Decision (CFSP) 2023/2287.
...
Output: data/consolidated/32023R2406.csv (the EU Journal consolidated CSV
...
FRAMEWORK_CELEX = "32023R2406"
CONSOLIDATED_RE = re.compile(r"^02023R2406-\d{8}$")
PROGRAM_KEY = "EU-NIGER"
# The regulation's Article 2 fund freeze; travel bans live in Decision
# (CFSP) 2023/2287.
...
# The never-used list body: one paragraph holding a bracketed horizontal
# ellipsis.
PLACEHOLDER = "[…]"
...
@click.command(help="Parse consolidated Regulation (EU) 2023/2406 into a CSV.")
...
def main(celex: str, source: Path | None) -> None:
    try:
        ...
        if CONSOLIDATED_RE.match(celex) is None:
            raise ParseError(f"not a consolidated 2023/2406 CELEX: {celex!r}")
