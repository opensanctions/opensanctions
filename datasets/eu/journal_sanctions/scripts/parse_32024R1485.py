...
CONSOLIDATED_RE = re.compile(r"^02024R1485-\d{8}$")
PROGRAM_KEY = "EU-RUS"
...
def check_registry() -> None:
    ...
    for schema_name in [part[2] for part in PARTS] + list(SCHEMA_PINS.values()):
...
def main(celex: str, source: Path | None) -> None:
    try:
        ...
        if CONSOLIDATED_RE.match(celex) is None:
            raise ParseError(f"not a consolidated 2024/1485 CELEX: {celex!r}")
