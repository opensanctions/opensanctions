...
CONSOLIDATED_RE = re.compile(r"^02018R1542-\d{8}$")
PROGRAM_KEY = "EU-CHEM"
...
def parse_part(roman: str, part: str, schema: str, table: Element) -> list[Row]:
    ...
    for tr in table_body(f"{roman}.{part}", table, HEADER):
        ...
        row.start_date = verbatim_date(cell_line(cells[3], ctx), ctx)
        rows.append(row)
    return rows
...
def main(celex: str, source: Path | None) -> None:
    try:
        ...
        if CONSOLIDATED_RE.match(celex) is None:
            raise ParseError(f"not a consolidated 2018/1542 CELEX: {celex!r}")
