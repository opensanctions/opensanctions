"""Parse consolidated Decision 2010/573/CFSP (Transnistria) into a CSV.
...
adopted for this regime, and the decision imposes only the Article 1 travel
restrictions — there is no fund-freeze article. The decision has a single
unnumbered annex ("ANNEX", the persons referred to in Article 1(1)) which
is currently empty — it prints one "…" placeholder paragraph. The parser
...
Output: data/consolidated/32010D0573.csv (the EU Journal consolidated CSV
...
ANNEX_TITLE = "ANNEX"
ANNEX_SUBTITLE = "Persons referred to in Article 1(1)"
...
def check_empty_annex(block: Element) -> None:
    """Accept the empty list: its subtitle and one "…" placeholder."""
    subtitles = 0
    placeholders = 0
...
@click.command(help="Parse consolidated Decision 2010/573/CFSP into a CSV.")
