...
CONSOLIDATED_RE = re.compile(r"^02023R2147-\d{8}$")
PROGRAM_KEY = "EU-SDNZ"
...
def parse_row(roman: str, part: str, schema: str, tr: Element) -> Row:
    ...
    row.start_date = verbatim_date(scalar_cell(cells[4], ctx), ctx)
    return row
...
def main(celex: str, source: Path | None) -> None:
    try:
        ...
        if CONSOLIDATED_RE.match(celex) is None:
            raise ParseError(f"not a consolidated 2023/2147 CELEX: {celex!r}")
