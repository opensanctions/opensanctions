...
CONSOLIDATED_RE = re.compile(r"^02017R2063-\d{8}$")
PROGRAM_KEY = "EU-VEN"
...
def check_registry() -> None:
    ...
    if model.get("Person") is None:
        raise ParseError("unknown schema 'Person'")
...
def main(celex: str, source: Path | None) -> None:
    try:
        ...
        if CONSOLIDATED_RE.match(celex) is None:
            raise ParseError(f"not a consolidated 2017/2063 CELEX: {celex!r}")
