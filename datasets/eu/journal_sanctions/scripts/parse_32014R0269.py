...
CONSOLIDATED_RE = re.compile(r"^02014R0269-\d{8}$")
PROGRAM_KEY = "EU-UKR"
...
def check_registry() -> None:
    ...
    for schema_name in [part[2] for part in PARTS] + ["Company"]:
...
def main(celex: str, source: Path | None) -> None:
    try:
        ...
        if CONSOLIDATED_RE.match(celex) is None:
            raise ParseError(f"not a consolidated 269/2014 CELEX: {celex!r}")
