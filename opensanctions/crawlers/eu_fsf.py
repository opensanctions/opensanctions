...
def parse_entry(emitter, entry):
    ...
    entity.add('sourceUrl', entry.findtext('./default:regulation/default:publicationUrl', '',NS))

    sanction = emitter.make('Sanction')
    sanction.make_id(entity.id)
    sanction.add('entity', entity)
    sanction.add('authority', 'European Union')
    sanction.add('sourceUrl', entry.findtext('./default:regulation/default:publicationUrl', '', NS))
    program = jointext(entry.find('./default:regulation', NS).get('programme'), entry.find('./default:regulation', NS).get('numberTitle'),
                       sep=' - ')
    sanction.add('program', program)
    sanction.add('reason', entry.findtext('./default:remark', '', NS))
    sanction.add('startDate', entry.find('./default:regulation', NS).get('entryIntoForceDate'))
    ...
    for pnode in entry.findall('./default:identification[@identificationTypeCode="passport"]', NS):
    ...
    for node in entry.findall('./default:address', NS):
        address = jointext(node.get('street'),
                           node.get('city'),
