...
def parse_entry(group, rows):
    ...
    for row in rows:
        record.update({
            ...
            'country_of_birth': source.normalize_country(row.pop('Country of Birth')),
            'nationality': source.normalize_country(row.get('Nationality')),
        ...
        if row.get('Passport Details'):
            record['identities'].append({
                ...
                'country': source.normalize_country(row.get('Nationality'))
            })
